...
Repository Overview
This is the Resistive_SensorResistiveSoilSensor Arduino library - a resistive soil moisture sensor library for PlatformIO and Arduino IDE. Supports both PULL_UP and PULL_DOWN configurations.
...
How to Verify
PlatformIO: Run pio lib install then compile in each env:

pio run -e uno
pio run -e nodemcu
pio run -e esp32dev
PlatformIO: Run pio lib install ResistiveSoilSensor then compile in each env:
...
pio run -e esp32-s2
pio run -e stm32
Arduino: Include library and compile a sketch using ResistiveSoilSensor.h
...
PlatformIO: pio lib install Resistive_Sensorpio lib install ResistiveSoilSensor
Arduino IDE: Search "Resistive_SoilSensor"
Current Version
